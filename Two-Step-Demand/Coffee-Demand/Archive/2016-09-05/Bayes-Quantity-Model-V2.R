...
market_code = 501             # Give the market code for which the model is to be estimated,
...
cores = min(detectCores(logical=FALSE), 32)
...
clusterExport(cl,c('opt0', 'hess', 'xnames', 'll_homo_indv', 'ihessfun', 'hh_modified', 'nt'))
...
bhat0 = c(rep(0, np-1), 0);
beta0 = cbind(matrix(rep(0, nh*(np-1)), nrow=nh), 0);
Z = rep(1, nh);
A = 0;
Dbar = t(rep(0,np));
s2 = 2.93^2/np;
...
clusterExport(cl,c('ll_linear', 'hh_market_prod', 'XMat', 'np', 'nt', 
                   'nh', 'pif_linear', 'rwmhd_linear', 's2', 'a', 'b', 'hess_list'))
...
Tvd = rep(0, draws)
bindv = array(0, dim=c(draws, nh, (np+1)))
...
for (d in 1:totdraws){
  ...
  # Compute the Posterior Mean for T and variance of it. 
  That = mean(T0)
  Tv  = var(T0)
  ...
  # Parallel version
  clusterExport(cl, c('sig', 'bhat', 'beta0', 'T0', 'Tv'))
  beta_list = parLapply(cl, 1:nh, rwmhd_linear)
  b0 = matrix(unlist(beta_list), ncol=(np+1), byrow=TRUE)
  T0 = b0[,(np+1)]
  beta0 = b0[,1:np]
...
save(hh_index_list, bhatd, sigd, Thatd, Tvd, bindv, bnames, 
