...
source(paste(code_dir, 'mdc-functions-O1.R', sep=""))
...
save(opt0, hess, file = paste(input_dir, "/Homo-Hessian-O1.RData", sep=""))

# Evaluate Hessian at Fractional likelihood
invisible(clusterEvalQ(cl, load(paste(input_dir, "/Homo-Hessian-O1.RData", sep=""))))
hess_list = clusterEvalQ(cl, lapply(hh_list, ihessfun))
hess_list = unlist(hess_list, recursive=F)
save(opt0, hess, hess_list, file = paste(input_dir, "/Hessian-O1.RData", sep=""))
...
invisible(clusterEvalQ(cl, load(paste(input_dir, "/Hessian-O1.RData", sep=""))))
...
invisible(clusterEvalQ(cl, (beta_dt=data.table(matrix(rnorm(length(hh_list)*np, mean=0, sd=10), 
                                                      ncol=length(hh_list))))))
