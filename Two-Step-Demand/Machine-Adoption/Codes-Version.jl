...
# Initialize W(ν)
@everywhere W1 = zeros(Float64, nodes);
...
@everywhere function kden(θ1::Vector, θ0::Matrix, H::Matrix)
    cdist = MvNormal(θ1, H)
    denv = pdf(cdist, θ0)
...
@everywhere function WApprox(theta::Vector, thetav::Matrix, H::Matrix, wmat::Array{Float64,3})
    wts = kden(theta, thetav, H)'
...
function vfun!(W0_wh::Matrix, W0_rg::Matrix, ϑ::Vector, βp::Float64)
...
N = 300 # Number of lags to store and use for value function approximation.
...
for d=1:totdraws
    ...
    W0_wh[:] =  WApprox(theta1, thtild, H, wtild_wh); # Approximate the new W0_wh
    W0_rg[:] =  WApprox(theta1, thtild, H, wtild_rg); # Approximate the new W0_rg
    ...
    vfun!(W0_wh, W0_rg, ϑ, βp); # Compute the choice specific value for each observation
    l1 = llfun!();
   ...
   # Define the relevant parameters
    ...
    # Approximate W function
    W0_wh[:] =  WApprox(theta0, thtild, H, wtild_wh); # Approximate the new W0_wh
    W0_rg[:] =  WApprox(theta0, thtild, H, wtild_rg); # Approximate the new W0_rg
