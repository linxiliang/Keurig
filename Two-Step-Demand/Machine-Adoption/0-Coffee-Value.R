...
hhValFun<-function(i, n=1000){
  ...
  hh_retailers_temp[, zb := xmat %*% pref[as.character(i),1:28]]
  ...
  hh_agg = hh_agg[, .(av=sum(av*tprob)/sum(tprob), gv=sum(gv*tprob)/sum(tprob), pprob=pprob[1]), 
                  by = c("household_code", "week_end")]
  hh_agg[, `:=`(av = av*pprob, gv = gv*pprob)]
...
bds = cval_list[, quantile(mu_diff, c(0.00025, 0.99))]
cval_list = cval_list[mu_diff>=bds[1] & mu_diff<=bds[2],]
mean_gain = cval_list[, .(delta = mean(mu_diff), dvar = var(mu_diff)/.N, sdv = sd(mu_diff)), by = "week_end"]
setkey(mean_gain, week_end)
save(cval_list, mean_gain, file = paste(output_dir, "/HH-Util-Diff.RData", sep=""))
