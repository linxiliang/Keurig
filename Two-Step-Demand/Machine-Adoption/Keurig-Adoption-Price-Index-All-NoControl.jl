...
@everywhere ZMat = hh_adoption[:, [4; 12:17]]
...
function llfun(pvec, y::Vector)
    return sum(log(pvec).*y + log(1-pvec).*(1-y))
end
...
N_0 = 1000
thtild = [1.; -1.] .+ 10*rand(walkdistr, N_0);
stild = zeros(Float64, 3, N_0);
wtild = zeros(Float64, N_0);
for (k in 1:300)
    for i in 1:N_0
        thtild[:,i] = [1., -1.] + 10*rand(walkdistr)
