...
@everywhere ZMat = hh_adoption[:, [10; 12:16]]
...
@everywhere function WApprox(θ1::Vector, θ0::Matrix, H::Matrix, s1::Vector, s0::Matrix, h::Float64, wvec::Vector)
    ...
    pbar_n2 = 0.3168768*s1[1] + 0.6831232*s1[2]
    #pbar_n1 = 8.143757 + 0.938283*pbar_n2
    pbar_n1 = 8.4911092 + 0.9269870*pbar_n2
    sbar_n = 0.0795693 + 0.9577252*s1[3]
    # sdist = MvNormal([pbar_n1, pbar_n2, sbar_n], diagm([3.683^2, h, 0.6308^2]))
    sdist = MvNormal([pbar_n1, pbar_n2, sbar_n], diagm([7.134^2, h, 0.6308^2]))
    wts1 = pdf(sdist, s0)
    wts = wts0 .* wts1
    wts[:] = wts ./ sum(wts)
    return (sum(wts .* wvec))
...
writedlm("Data/Bayes-MCMC/Adoption-Coef-14500.csv", thatd) # Write the posterior average coefficients
writedlm("Data/Bayes-MCMC/Adoption-Coef-23000-dollar.csv", thatd) # Write the posterior average coefficients

writedlm("Data/Bayes-MCMC/Adoption-Coef-20000-Old-Data.csv", thatd) # Write the posterior average coefficients
writedlm("Data/Bayes-MCMC/Adoption-Coef-34944-Old-Data.csv", thatd) # Write the posterior average coefficients
writedlm("Data/Bayes-MCMC/Adoption-Coef-23363-Old-Data.csv", thatd) # Write the posterior average coefficients

writedlm("Data/Bayes-MCMC/Adoption-Coef-22384-Simulation-Both.csv", thatd) # Write the posterior average coefficients
writedlm("Data/Bayes-MCMC/Adoption-Coef-26782-Simulation-Both.csv", thatd) 
# True 4, -2, -7, 2, 1, 1.2, 1.3, 1.1, 0.5
# Write the posterior average coefficients
