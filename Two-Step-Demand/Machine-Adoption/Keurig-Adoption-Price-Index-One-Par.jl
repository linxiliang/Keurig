...
@everywhere function WApprox(θ1::Float64, θ0::Vector, H::Float64, s1::Vector, s0::Matrix, h::Float64, wvec::Vector)
    cdist = Normal(θ1, H)
    wts0 = pdf(cdist, θ0)
...
# Beta is 0.995 here 
# Scaling Version 2
@everywhere function W0V(θ1::Float64, s1::Vector, w::Float64)
    return log(exp(0.995*w) + exp((θ1 * (s1[1]/100) + coefun(W1coef, s1[3])/100)))
end

# For values of tstate and purchased - function of delta
@everywhere function pfun(θ1::Float64, θ0::Vector, H::Float64, XVec::Vector, s0::Matrix, h::Float64, wvec::Vector, W1coef::Vector)
    ex0 = exp(0.995 * WApprox(θ1, θ0, H, XVec, s0, h, wvec))
    ex1 = exp((θ1 * XVec[1]/100 + coefun(W1coef, XVec[3])/100))
    return(ex1/(ex0+ex1))
end
...
function pfun!(θ1::Float64, θ0::Vector, H::Float64, Xmat::Matrix, s0::Matrix, h::Float64, wvec::Vector, W1coef::Vector)
...
N_0 = 1000
thtild = -5 .+ vec(30*rand(walkdistr, N_0)[1,:]);
stild = zeros(Float64, 3, N_0);
wtild = zeros(Float64, N_0);
for (k in 1:100)
    for i in 1:N_0
        thtild[i] = -5. + 30*rand(walkdistr)[1]
...
writedlm("Data/Bayes-MCMC/Adoption-Coef-One-Par-13565.csv", thatd[1:13565, :]) # Write the posterior average coefficients
