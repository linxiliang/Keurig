...
true_theta = [6., -2.]
true_gamma = [-2., 2., 1., 1.2, 1.3, 1.1, 0.5]
...
aprob = zeros(Float64, length(purch_vec))
for i in 1:length(purch_vec)
    ex0 = exp(0.995 * WApprox(vec(XMat[i,:]), rstate, h, wtild0)/100)
    ex1 = exp((true_theta[1] + true_theta[2] * XMat[i,1] + coefun(W1coef, XMat[i,3]))/100)
    aprob[i] = ex1/(ex0+ex1)
    oprob = aprob[i] * pprob[i]
    if rand() <= oprob
        spurch[i] = 1
    end
end
...
N_0 = 1000
thtild = [5.; -5.] .+ 10*rand(walkdistr, N_0)[1:2,:] ;
stild = zeros(Float64, 3, N_0);
wtild = zeros(Float64, N_0);
for (k in 1:300)
    for i in 1:N_0
        thtild[:,i] = [5., -5.] + 10*rand(walkdistr)[1:2]
...
writedlm("Data/Bayes-MCMC/Simulation-Both-Intercepts-30000.csv", thatd)
writedlm("Data/Bayes-MCMC/Estimation-Both-Intercepts-45000.csv", thatd)


## MCMC Draws Gibbs Version
for d=1:totdraws
    if d==3000
        H = H/10
        #sigs = eye(9)/30
    ...
    # Gibbs sampling
    theta1 = theta0 + rand(walkdistr)[1:2]
    ...
    pval1 = Array(pval)
    l1 = llfun(pval1, hvec0, purch_vec);
    ...
    pval0 = Array(pval)
    l0 = llfun(pval0, hvec0, purch_vec); 

    # Compute alpha 1
    alpha1 = min(1, postd(l1, l0, [theta1;gamma0], [theta0;gamma0], bhat, sigb))
    if rand() <= alpha1
        theta0 = theta1
        pval0 = pval1
    end
    ...
    # Draw Gamma
    gamma1 = gamma0 + rand(walkdistr)[3:end]
    
    # New participation probability
    hvec1 = participation(gamma1)
    
    # New likelihood
    l1 = llfun(pval0, hvec1, purch_vec);
    l0 = llfun(pval0, hvec0, purch_vec);
    
    alpha2 = min(1, postd(l1, l0, [theta0;gamma1], [theta0;gamma0], bhat, sigb))
    ...
    if ((d > burnin) && (d % thin == 0))
        ...
        thatd[indx, :] = [theta0;gamma0];
    end
                                            
    # Indicate the progress
    println([l0, l1])
    println([theta0;gamma0])
