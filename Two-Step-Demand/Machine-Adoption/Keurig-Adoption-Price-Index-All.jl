...
@everywhere function statewt(s1::Vector, s0::Matrix, h::Float64)
    pbar_n2 = 0.3617623*s1[1] + 0.6382377*s1[2]
    #pbar_n1 = 8.143757 + 0.938283*pbar_n2
    pbar_n1 = 8.5120709 + 0.9389002*pbar_n2
    sbar_n = 0.006759535 + 0.9097726*s1[3]
...
gamma0 = param0[3:end]
KVec0 = ZMat * gamma0
cdist = MvNormal(theta0, H)
wt0 = pdf(cdist, thtild)
ws = SharedArray(Float64, length(purch_vec), length(wtild))
@sync @parallel for i in 1:nobs
    ws[i,:]=statewt(vec(XMat[i,:]), stild, h)
end
ws  = Array(ws)
wsv = SharedArray(Float64, length(purch_vec))
...
for d=9999:totdraws
    ...
    @sync @parallel for i in 1:nobs
          wsv[i]=statewt(vec(XMat[i,:]), s_i, h)
    end
    
    # Compute relevant values
    KVec1 = ZMat * gamma0
    cdist = MvNormal(theta1, H)
    wt1 = pdf(cdist, thtild)
    
    # Approximate the Value function
    w_i = WApprox(wt1, statewt(s_i, stild, h), wtild)
    ...
    # Communication cost !!!!!!!! Crazy!!!!
    # Compute likelihood for the new
    wt1 = pdf(cdist, thtild)
    l1 = llfun(theta1, wt1, ws, XMat, wtild, AVec, KVec1, purch_vec) # Compute the Likelihood
    
    # Compute likelihood for the old draw
    wt0 = pdf(cdist, thtild)
    l0 = llfun(theta0, wt0, ws, XMat, wtild, AVec, KVec0, purch_vec) # Compute the Likelihood
...
writedlm("Data/Bayes-MCMC/Adoption-Coef-NoArrival.csv", thatd) # Write the posterior average coefficients
