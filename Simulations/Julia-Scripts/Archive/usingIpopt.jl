...
function grad_f(x::Vector){
  [2 for i in x]
}

prob = createProblem(0, 0, 0, m, g_L, g_U, 8, 10,
                     eval_f, eval_g, eval_grad_f, eval_jac_g, eval_h)
...
@setNLObjective(m, Min, exp(x)+y^2)
...
@setNLObjective(m, Min, LL(x1, x2, x3, x4))
solve(m)
...
println("x = ", getValue(param0), " y = ", getValue(y))
...
lprob = Array(Float64, sum(periods))
\
    j = 0     
    for i = (1:3:(3*sum(periods)))
       j+=1
       #println("i = $i, j=$j")
       lprob[j]= purchases[i]*log(expu[i]/sum(expu[i:(i+2)])) + 
              purchases[i+1]*log(expu[i+1]/sum(expu[i:(i+2)])) + 
              purchases[i+2]*log(expu[i+2]/sum(expu[i:(i+2)]))
    end
    ll = sum(lprob)
