...
vfun_ev <- function(m0, rho, alpha, u0, price){
  if (length(u0)==1){
    ...
    # solve for e1
    e1v = uniroot(ovfun, c(cmin, cmax), m0=m0, rho=rho, alpha=alpha[1:j], 
                  u0=u0[1:j], price=price[1:j], tol=1e-16)
...
nsim = 5000
sim_dt = as.list(1:nsim)
par_dt = as.list(1:nsim)
for (k in 1:nsim){
  # households
  zx =  rnorm(nprod, mean=3)
  alpha = c(rnorm(1, mean=4, sd=0.1), rnorm(1, mean=2, sd=0.25))
  alpha = cbind(1-keurig, keurig)%*%alpha
  rho = rnorm(1, mean=-0.2, sd=0.025)
  par_dt[[k]] = c(zx, alpha, rho)
  
  # Create the dataset
  panel = data.table(expand.grid(hh=k, t = 1:nt, brand = 1:nprod))
...
sim_dt = rbindlist(sim_dt)
save(sim_dt, par_dt, file = "~/Keurig/Data/Simulation-Data/Simulated-Coffee-5Prod.RData")