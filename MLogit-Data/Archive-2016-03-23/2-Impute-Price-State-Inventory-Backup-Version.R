...
hh_retailer_list = unique(purchases[selected==1, .(retailer_code)])
...
coffee_trips = unique(purchases[selected==1, trip_code_uc])
...
brand_type_sales = purchases[selected==1, .(brand_sales = sum(total_price_paid-coupon_value)), 
...
selected_brand_list = unique(brand_type_sales[(keurig==1 & cumshare<=0.96) | (keurig==1 & cumshare<=0.92), brand_descr]) 
top_keurig_brands = brand_type_sales[keurig==1 & brand_descr!="CTL BR" & brand_share>=0.04, brand_descr]
...
retailer_prices = purchases[selected==1, .(revenue = sum(total_price_paid - coupon_value),
...
retailer_9005 = retailer_prices[retailer_code==9005, ]
retailer_9006 = retailer_prices[retailer_code==9006, ]
retailer_9099 = retailer_prices[retailer_code==9099, ]
...
retailer_type_sales = purchases[selected==1, .(retailer_sales = sum(total_price_paid-coupon_value),
                                               npurch = .N), 
...
rms_retailer_list = unique(store_codes[nretailer_codes==1, .(retailer_code = as.integer(retailer_code))])
...
retailer_type_sales[, selected := as.integer((keurig==1&retailer_rank<=25)|in_rms==TRUE)]
...
selected_channels = retailers[retailer_code%in%retailer_type_sales[selected==1 & retailer_rank<=25, retailer_code], 
                              unique(channel_type)]
#---------------------------------------------------------------------------------------------------#
...
product_panel = purchases[channel_type %in% selected_channels, 
                          .(first_week_end = min(week_end),
                            ...
                            quantity = sum(quantity)),
                          by = c("dma_code", "retailer_code", "brand_descr", "keurig", "size1_amount")]
...
setkeyv(product_panel, c("dma_code", "retailer_code", "week_end", "brand_descr", "keurig", "size1_amount"))

# Create the market, retailer, brand, size and week_end panel for top retailers (inclusive of RMS for testing)
top_retailers  = unique(retailer_type_sales[selected==1 & retailer_rank<=25, retailer_code])
...
setkeyv(retailer_panel_1, c("dma_code", "retailer_code", "brand_descr", "keurig", "size1_amount", "week_end"))
setkeyv(product_panel, c("dma_code", "retailer_code", "brand_descr", "keurig", "size1_amount", "week_end"))
nameorder = names(retailer_panel_1)
retailer_panel_1 = retailer_panel_1[product_panel[, .(dma_code, retailer_code, brand_descr, keurig, 
...
for (dma_n in top_dma_list){
  i = i+1
  dma_region_list = unique(purchases[dma_code == dma_n, unique(region_code)])
...
for (i in top_retailers){
  dt_temp = retailer_panel_1[retailer_code==i, ] 
  ...
  retailer_panel_1[retailer_code==i, p_imputed_1 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==1, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_2 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==2, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_3 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==3, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_4 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==4, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_5 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==5, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_6 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==6, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_7 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==7, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_8 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==8, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_9 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==9, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_10 := predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==10, rsquared := summary(regout)$r.squared]
  ...
  regout$xlevels[["factor(size1_amount)"]] <- union(regout$xlevels[["factor(size1_amount)"]], levels(factor(dt_temp$size1_amount)))
  regout$xlevels[["factor(brand_descr)"]] <- union(regout$xlevels[["factor(brand_descr)"]], levels(factor(dt_temp$brand_descr)))
  retailer_panel_1[retailer_code==i, p_imputed_11:= predict(regout, dt_temp)]
  retailer_R2[retailer_code==i & model_code==11, rsquared := summary(regout)$r.squared]
...
for (i in 1:nrow(retailer_R2)){
  ...
  retailer_panel_1[retailer_code==retailer_R2[i, retailer_code]&is.na(price), price:=focal_var]
...
trips = trips[top_95_retailer==1 | npurch>=1, ]
trips[, ctrip := as.integer(npurch>=2 | top_95_retailer==1 | coffee_trip==1)]
...
trips[retailer_trips==0, `:=`(retailer_trips = ifelse(ctrip==0, as.integer(0), as.integer(1)),
...
week_trips[retailer_trips == 0, retailer_trips:=1]
...
hh_retailer_trips = trips[(retailer_trips>=1) & (retailer_filter==1 | coffee_trip==1), ]
hh_retailer_trips[, retailer_filter:=1:.N, by = c("household_code", "week_end")]
hh_retailer_trips[coffee_trip==0, retailer_filter:=as.integer(retailer_filter==max(retailer_filter)),
                  by = c("household_code", "week_end")]
hh_retailer_trips = hh_retailer_trips[coffee_trip==1 | retailer_filter==1, ]
...
focal_purch = purchases[selected==1 | household_code %in% hh_list[, household_code], ]
...
focal_purch_last = focal_purch[plast==1, ]
...
# Generate the states
focal_purch_last[brand_lag==brand_descr_modified, brand_cum := brand_cum + quantity * size1_amount]
focal_purch_last[brand_lag!=brand_descr_modified, brand_cum := quantity * size1_amount]
...
hh_panel_temp = hh_panel[norder==1 & (!is.na(is_lag) | coffee_trip==1), 
...
hh_rate = purchases[, .(quantity = quantity * size1_amount), by = .(household_code, purchase_date, ptype)]
...
hh_rate_first = hh_rate[, .(ptype=ptype[1]), by = "household_code"]
...
setnames(hh_rate_first, "ptype", "ptype_lag")
setkey(hh_rate, household_code, ptype_lag)
setkey(hh_rate_first, household_code, ptype_lag)
hh_rate_first = hh_rate[,.(household_code, ptype_lag, crate)][hh_rate_first, nomatch=0L]
...
setkey(hh_rate_first, household_code)
setkey(hh_inventory, household_code)
#Households with less than 3 purchase dates are dropped.
hh_inventory = hh_rate_first[, .(household_code, crate)][hh_inventory, nomatch=0L] 
hh_inventory[!is.na(i.crate), crate:=i.crate]
hh_inventory[, i.crate:=NULL]
...
inventory_func <- function(hh_code){
  ...
  for (i in 1:nobs){
    if (i==1 | date_diff>=52) {
      ...
      if (is.na(hpurch[i-1, ptype_lag]) | is.na(hpurch[i, ptype_lag])){
...
hh_inventory = hh_inventory[first_panel_purch_week - week_end <= 26, ]
hh_inventory = hh_inventory[, .(household_code, week_end, inv_all, inv_type, inv_brand_type)]
rm(inventory_list)
...
hh_panel[is.na(kholding), kholding:=0]
...
hh_panel = hh_panel[, .(dma_code, retailer_code, household_code, kholding, week_end, norder, trip_code_uc, 
                        coffee_trip, is_lag, brand_lag, brand_cum, ptype_lag, inventory)]
...
hh_prod_panel = hh_prod_panel[kholding==1 | (kholding == 0 & keurig == 0), ]
...
hh_prod_panel[brand_lag==brand_descr_modified & brand_lag!="OTHER", `:=`(is_lag = 1)]
hh_prod_panel[brand_lag!=brand_descr_modified | brand_lag!="OTHER", `:=`(is_lag = 0, brand_cum = 0)]

# Input the number of periods of consumption, and impute the new inventory...
# Method 1 - Receive a scrape value for the inventory at hand... (can also pay a value to convert the inventory).
...
hh_rate = hh_rate[ptype_lag=="KEURIG" | ptype_lag=="OTHER", ]
hh_rate[, keurig_c:=as.integer(ptype_lag=="KEURIG")]
setkey(hh_prod_panel, household_code, keurig_c)
setkey(hh_rate, household_code, keurig_c)
hh_prod_panel = hh_rate[,.(household_code, keurig_c, crate)][hh_prod_panel]
hh_prod_panel[, acrate:=mean(crate, na.rm=TRUE), by = "household_code"]
hh_prod_panel[is.na(crate), crate := acrate]
hh_prod_panel[, acrate:=NULL]
hh_prod_panel = hh_prod_panel[!is.na(crate), ] # If still missing, probably someone who adopted other machine and never consumed ground.
...
hh_prod_panel = focal_purch[, .(trip_code_uc, brand_descr, keurig, size1_amount, quantity)][hh_prod_panel]
hh_prod_panel[is.na(quantity), quantity := 0]
hh_prod_panel[, sum_quantity:=sum(quantity), by ="trip_code_uc"]
hh_prod_panel[sum_quantity==0&brand_descr=="0NOTHING", quantity:=1]
...
dma_hh_list = unique(hh_prod_panel[dma_code==501, household_code])
hh_market_prod = hh_prod_panel[household_code %in% dma_hh_list, ]
hh_market_prod[,`:=`(dma_code=NULL, retailer_code=NULL, coffee_trip=NULL, brand_lag=NULL, 
...
hh_market_prod = hh_market_prod[npurch==1, ]
...
hh_market_prod[ (!(brand_descr_c %in% top_selling_brands)) & brand_descr_c!="0NOTHING", brand_descr_c:="OTHER"]

# Modify the brand to create idiosyncratic match values for top Keurig Brands
hh_market_prod[keurig_c==1 & brand_descr %in% top_keurig_brands, 
...
hh_info_panel = hh_info_panel[year_filter==1, ]
setkey(hh_info_panel, household_code)
hh_demo = hh_household_code[hh_info_panel[,.(household_code, overall_rate, inc25, inc40, inc50, inc60, inc70,
...
hh_market_prod[, `:=`(inventory = inventory/10, kinventory = kinventory/10, sinventory = sinventory/10,
                      ...
                      brand_cum = brand_cum/100)]
hh_market_prod[, `:=`(invsq = inventory^2, sinvsq = sinventory^2, brand_cum_sq = brand_cum^2)]
name_order = c("hh", "t", "brand", "coef1", bvars, "keurig", "coef2", "price", "inventory", 
               "kinventory", "sinventory", "brand_lag", "brand_cum_log", "purchased")
