...
# No variety seeking
bhatd[4001:10000,27:28] = exp(bhatd[4001:10000, 27:28])/(1+exp(bhatd[4001:10000, 27:28]))
bhatd = colMeans(bhatd[4001:10000, ])
n = dim(pref)[1]
for (i in 1:n){
  pref[i,1:28] = bhatd
}
...
hhValFun<-function(i){
  ...
  for (j in 1:30){
    ...
    hh_retailers_temp[licensed==0, `:=`(fil0_2 = as.integer(U0>max(UE))), by = .(idx)]
    ...
    hh_retailers_temp[thirdp==0, `:=`(fil0_3 = as.integer(U0>max(UE))), by = .(idx)]
  ...
  hh_agg[, `:=`(uall1=uall1_cum/30, uall2=uall2_cum/30, uall3=uall3_cum/30, ugrd=ugrd_cum/30)]
...
save(cval_list, file = paste(output_dir, "/HH-Homo-Diff-Type.RData", sep=""))

load(paste(output_dir, "/HH-Homo-Diff-Type.RData", sep=""))
...
write.csv(hw_market_panel[,.(household_code, hware, ntrip, t, price, price_avg, price_avgn,
          ...
          file = paste(output_dir,"/Homo-MU-Panel.csv",sep=""), row.names = FALSE)
